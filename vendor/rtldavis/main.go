...
var (
    ...
    tol               int            // -tol = max bit errors tolerated in preamble match
...
var (
    ...
    flag.IntVar(&tol, "tol", 0, "max bit errors tolerated in 16-bit preamble match (0 = exact match only)")
    ...
    log.Printf("tr=%d fc=%d ppm=%d gain=%d maxmissed=%d ex=%d tol=%d receiveWindow=%d actChan=%d maxChan=%d", tr, fc, ppm, gain, maxmissed, ex, tol, receiveWindow, actChan[0:maxChan], maxChan)
...
func main() {
    var sdrIndex int = -1
    p := protocol.NewParser(14, *transmitterFreq, tol)
    p.Cfg.Log()
