...
func NewParser(symbolLength int, tf string, preambleTolerance int) (p Parser) {
	...
	if tf == "EU" {
		...
		if tf == "NZ" {
			...
			p.channels = []int{
				// davis-clientraw-us-channels -- this project's /calibrate tool
				// regex-replaces the single line below in place (same convention
				// as the EU table above); keep it as exactly one line ending in
				// a "//" comment. Original values from Paul Anderson and Rich T.
				902419338, 902921088, 903422839, 903924589, 904426340, 904928090, 905429841, 905931591, 906433342, 906935092, 907436843, 907938593, 908440344, 908942094, 909443845, 909945595, 910447346, 910949096, 911450847, 911952597, 912454348, 912956099, 913457849, 913959599, 914461350, 914963100, 915464850, 915966601, 916468351, 916970102, 917471852, 917973603, 918475353, 918977104, 919478854, 919980605, 920482355, 920984106, 921485856, 921987607, 922489357, 922991108, 923492858, 923994609, 924496359, 924998110, 925499860, 926001611, 926503361, 927005112, 927506862, // US freq per 20190326
			}
		}
