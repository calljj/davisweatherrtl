...
// fuzzyIndex returns the index of the first position in text where pattern
// matches with at most maxErrors mismatched bytes (bits, in our case: each
// byte is a quantized 0/1 symbol). Unlike PreambleFinder.next, which requires
// a bit-perfect match, this tolerates noise in the preamble caused by
// marginal SNR, residual frequency/timing error, etc. Returns -1 if no
// position matches within tolerance.
func fuzzyIndex(pattern, text []byte, maxErrors int) int {
...
func (d *Demodulator) Search() (indexes []int) {
	for symbolOffset, slice := range d.slices {
		...
		idx := 0
		for {
			if d.Cfg.PreambleTolerance > 0 {
				idx = fuzzyIndex(d.Cfg.PreambleBytes, slice[offset:], d.Cfg.PreambleTolerance)
			} else {
...
func NewPacketConfig(bitRate, symbolLength, preambleSymbols, packetSymbols int, preamble string, preambleTolerance int) PacketConfig {
	...
	cfg.PreambleTolerance = preambleTolerance
